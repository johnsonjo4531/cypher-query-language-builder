...
const isValidPropsWhitelist = (props, whitelist) => props &&
    whitelist &&
...
class CypherHelper {
    constructor(config = {}) {
        this.query = Object.assign((strings, ...params) => {
            ...
            setters({ name, properties, propsWhitelist }) {
                if (!properties || !propsWhitelist) {
                    throw new TypeError("'properties' and 'propsWhitelist' keys must be defined");
                }
                if (properties &&
                    !isValidPropsWhitelist(properties, propsWhitelist)) {
                    throw new TypeError("propsWhitelist key must be defined as an array of allowed keys as strings for safety reasons when properties are defined");
                }
                if (!name) {
                    throw new TypeError("'name' must be defined in config object");
                }
